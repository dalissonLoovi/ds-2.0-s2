...
- **actionChrome:** pending|rejected Button: showLabel=false (chevron-only); keep label prop for accessible name
- **rejectedChipFill:** rejected ChipTag intent=danger with canvas fill override to color/background-feedback-primary/warning
...
expose label and status; pending/rejected Button chrome is chevron-only (showLabel=false) but label prop supplies the accessible name and must stay keyboard-accessible; in-review/approved are informational (no focus unless an action is added).
